...
SERVICE="next-shift-memory-sync"
...
SOURCE="/home/patrick/next-shift/services/memory_sync_runtime"
...
gcloud run deploy "${SERVICE}" \
    ...
    --update-env-vars="ADVISOR_MODEL=${MODEL}" \
    --quiet
...
LIVE_MODEL="$(gcloud run services describe "${SERVICE}" \
    ...
    --region="${REGION}" \
    --format=json \
    | jq -r '.spec.template.spec.containers[0].env[]? | select(.name=="ADVISOR_MODEL") | .value')"

if [[ "${LIVE_MODEL}" != "${MODEL}" ]]; then
    printf 'ERROR: Memory advisor model mismatch: %s\n' "${LIVE_MODEL}"
    exit 1
fi

printf 'MEMORY_ADVISOR_DEPLOY_OK=1\n'
printf 'ADVISOR_MODEL=%s\n' "${LIVE_MODEL}"
