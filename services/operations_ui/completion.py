...
def _token(audience: str) -> str:
    ...
    if not token:
        raise RuntimeError(
            "Unable to obtain service identity token"
        )
    ...
    if response.status_code >= 400:
        raise RuntimeError(
            str(
                ...
                or f"Downstream status {response.status_code}"
            )
        )

    if not isinstance(payload, dict):
        raise RuntimeError(
            "Downstream service returned invalid payload"
        )

    return payload


def record_trusted_completion(
    issue_id: str,
) -> dict[str, Any]:
    return _post(
