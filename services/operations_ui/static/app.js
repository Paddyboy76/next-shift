...
async function loadIntelligence() {
  ...
  document.querySelector("#intelligence-panel").innerHTML = [
    `<div class="platform-fact"><strong>${escapeHtml(value.authority)}</strong> · ${escapeHtml(value.sample_size)} historical synthetic issues · Memory Bank ${escapeHtml(value.memory_bank?.status || "UNKNOWN")}<br>Generated by ${escapeHtml(value.generated_by?.provider || "unknown")} ${escapeHtml(value.generated_by?.model || "")}</div>`,
    ...recommendations.map((item) => `<div class="platform-fact"><strong>${escapeHtml(item.pattern)}</strong><br>${escapeHtml(item.recommended_change)}<br><span class="subtle-note">Why: ${escapeHtml(item.why_it_matters)} · Scope: ${escapeHtml(item.affected_scope)} · Expected: ${escapeHtml(item.expected_improvement)} · Confidence: ${escapeHtml(item.confidence)}</span><br><span class="subtle-note">Evidence: ${escapeHtml((item.evidence || []).join(", "))}<br>Memory records: ${escapeHtml((item.memory_records || []).join(", ") || "none on first refresh")}</span></div>`),
    `<div class="platform-fact">Provenance: ${escapeHtml(value.provenance?.source_collection || "handover_issues")} · generated ${escapeHtml(shortTime(value.generated_at))}<br>Current-state authority: ${escapeHtml(value.current_state_authority)} · workflow mutation: ${escapeHtml(value.may_mutate_workflow)}<br>Memory: ${escapeHtml(value.memory_bank?.memory_name || "not yet synchronized")}</div>`,
  ].join("");
}
...
async function loadPlatform() {
  ...
  document.querySelector("#platform-panel").innerHTML = `
    ...
    <div class="platform-fact"><strong>Managed Agent Identity</strong> · ${escapeHtml(value.agent_runtime.identity)}<br>Registry: ${escapeHtml(value.registry.agent)} · ${escapeHtml(value.registry.service)} · ${escapeHtml(value.registry.verification)}</div>
