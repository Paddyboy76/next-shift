...
STATUS_META = {
    "PENDING": {
        "icon": "schedule",
        "label": "Queued",
        "detail": "Waiting for Human Reach delivery",
        "color": "#5F6368",
    },
    "DELIVERED": {
        "icon": "notifications_active",
        "label": "Delivered",
        "detail": "Waiting for frontline acknowledgement",
        "color": "#1A73E8",
    },
    "ACKNOWLEDGED": {
        "icon": "how_to_reg",
        "label": "Acknowledged",
        "detail": "A frontline responder has accepted the work",
        "color": "#0F9D58",
    },
    "BLOCKED": {
        "icon": "report_problem",
        "label": "Blocked",
        "detail": "Human assistance or dependency resolution is required",
        "color": "#D93025",
    },
    "COMPLETION_CLAIMED": {
        "icon": "task_alt",
        "label": "Completion claimed",
        "detail": "Trusted evidence and independent verification are still required",
        "color": "#B06000",
    },
}
...
def _decorated(
    *,
    ...
    payload: dict[str, Any] = {
        "startIcon": _material_icon(icon, icon_alt),
        "topLabel": top_label,
        "text": text,
        "wrapText": True,
    }
...
def _status_widget(delivery: dict[str, Any]) -> dict[str, Any]:
    ...
    meta = STATUS_META.get(
        status,
        {
            ...
            "detail": "Review Human Reach delivery state",
            "color": "#5F6368",
        },
    )

    label = escape(str(meta["label"]))
    detail = escape(str(meta["detail"]))
    color = str(meta["color"])

    return _decorated(
        icon=str(meta["icon"]),
        icon_alt=label,
        top_label="HUMAN REACH STATUS",
        text=(
            f'<font color="{color}"><b>{label}</b></font> · {detail}'
        ),
        bottom_label=(
            "Delivery state is separate from verified operational closure"
        ),
    )
...
def _last_response_widget(delivery: dict[str, Any]) -> dict[str, Any] | None:
    ...
    actor = _text(
        delivery.get("last_response_display_name"),
        "Frontline responder",
    )
    ...
    return _decorated(
        icon="person_check",
        icon_alt="Frontline response",
        top_label="LAST HUMAN RESPONSE",
        text=f"<b>{escape(action.replace('_', ' ').title())}</b> · {actor}",
        bottom_label=at,
    )


def _response_buttons(
    delivery: dict[str, Any],
) -> list[dict[str, Any]]:
    status = str(delivery.get("delivery_status", "PENDING"))
    delivery_id = str(delivery.get("delivery_id", ""))

    if status not in {"DELIVERED", "ACKNOWLEDGED"}:
        return []
...
def work_card(delivery: dict[str, Any]) -> dict[str, Any]:
    ...
    assignment_widgets = [
        _decorated(
            icon="person",
            icon_alt="Assigned team or person",
            top_label="WHO",
            text=_text(delivery.get("who"), "Frontline team"),
        ),
        _decorated(
            icon="assignment",
            icon_alt="Operational task",
            top_label="WHAT",
            text=_text(delivery.get("what"), "Operational work"),
        ),
        _decorated(
            icon="location_on",
            icon_alt="Work location",
            top_label="WHERE",
            text=_text(delivery.get("where"), "Location pending"),
        ),
        _decorated(
            icon="confirmation_number",
            icon_alt="Work order",
            top_label="WORK ORDER",
            text=_text(delivery.get("work_order"), delivery_id or "Pending"),
            bottom_label=f"Next Shift issue {escape(delivery_id)}",
        ),
    ]
    ...
    human_widgets: list[dict[str, Any]] = [
        ...
        _decorated(
            icon="forum",
            icon_alt="Google Chat destination",
            top_label="ROUTED TO",
            text=route,
            bottom_label="Resolved deterministically from the specialist owner",
        ),
    ]
    ...
    governance_widgets: list[dict[str, Any]] = [
        _decorated(
            icon="account_tree",
            icon_alt="Operational workflow state",
            top_label="OPERATIONAL WORKFLOW",
            text=(
                "<b>ACTION_PENDING</b> · Human response is tracked separately"
            ),
            bottom_label="Firestore remains authoritative operational truth",
        ),
        _decorated(
            icon="verified_user",
            icon_alt="State Authority protection",
            top_label="CLOSURE BOUNDARY",
            text=(
                "A human completion claim does <b>not</b> close this issue. "
                "Trusted evidence must move it to VERIFYING, then the "
                "independent verifier controls CLOSED."
            ),
        ),
    ]
    ...
    if console_button is not None:
        governance_widgets.append(
            {
                "buttonList": {
                    "buttons": [console_button],
                }
            }
        )
    ...
    return {
        ...
        "card": {
            "header": {
                "title": issue_title,
                "subtitle": f"Next Shift Human Reach · {owner}",
            },
            "sectionDividerStyle": "SOLID_DIVIDER",
            "sections": [
                {
                    "header": "Operational assignment",
                    "widgets": assignment_widgets,
                },
                {
                    "header": "Human Reach",
                    "widgets": human_widgets,
                },
                {
                    "header": "Governed completion",
                    "collapsible": True,
                    "uncollapsibleWidgetsCount": 1,
                    "widgets": governance_widgets,
                },
            ],
        },
    }
